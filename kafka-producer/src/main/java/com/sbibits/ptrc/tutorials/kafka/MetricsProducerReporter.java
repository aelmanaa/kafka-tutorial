...
public class MetricsProducerReporter implements Runnable {
  ...
  private final Set<String> metricsNameFilter =
      Sets.set("record-queue-time-avg", "record-send-rate", "records-per-request-avg",
          "request-size-max", "network-io-rate", "record-queue-time-avg", "incoming-byte-rate",
          "batch-size-avg", "response-rate", "requests-in-flight");

  public MetricsProducerReporter(final Producer<Long, String> producer) {
  ...
  private void displayMetrics(Map<MetricName, ? extends Metric> metrics) {
    ...
    metricsDisplayMap.entrySet().forEach(entry -> {
      ...
      builder.append(String.format(Locale.US, "%50s%25s\t\t%,-10.2f\t\t%s\n", name,
          metricPair.metricName.name(), metricPair.metric.value(),
